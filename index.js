...
export default class Swiper extends Component {
  ...
  measure(){
    ...
    /*return new Promise((resolve,reject)=>{
      this.refScrollView.measure((fx, fy, width, height, px, py) => {
        /!**
         * console.log("width:" + width); //控件宽
         console.log("height:" + height);//控件高
         console.log("fx:" + fx); //距离父控件左端 x的偏移量
  ...
  initState (props, updateIndex = false) {
    ...
    // console.info("init this.internals",this.internals);
    return initState;
  }
...
*/
  ...
  updateIndex = (offset, dir, cb) => {
    ...
    // if(!this.internals.offset && !this.objUpdateIndex){
    if(!this.internals.offset){
      /*this.objUpdateIndex = {};
      this.objUpdateIndex.offset = offset;
      this.objUpdateIndex.dir = dir;
      this.objUpdateIndex.cb = cb;*/

      return;
    }
   /* else if(!offset){
      offset = this.objUpdateIndex.offset;
      dir = this.objUpdateIndex.dir;
      cb = this.objUpdateIndex.cb;
    }*/

    const state = this.state;
    let index = state.index;

    // console.info("offset dir=" + dir + " index=" + index,offset);
    // console.info("this.internals.offset[dir]=",this.internals);
    ...
    // console.info("Math.round(diff / step)",Math.round(diff / step));
    // console.info("this.internals step="+step+" index=" + index + " diff=" + diff,this.internals);
    if (this.props.loop) {
      if (index <= -1) {
        index = state.total - 1
   ...
   * @param  {bool} animated
   * @param  {bool} isPosi // 是否滑到指定位置 默认false
   */

  scrollBy = (index, animated = true,isPosi = false) => {
    // console.info("this.state index=" + index,this.state);
    // console.info("this.internals.isScrolling",this.internals.isScrolling);

    // console.info("state loop:" + this.props.loop + " index:" + index,this.state);
    if(index == this.state.index && !this.props.loop)
    {
      return;
    }
    ...
    const state = this.state;
    const diff = isPosi ? index
      : (this.props.loop ? 1 : 0) + index + this.state.index
    let x = 0;
    let y = 0;
    ...
    if (Platform.OS !== 'ios') {
      ...
      // console.info("state  diff=" + diff,state);
   ...
   * 滚动到上一页
   */
  scrollPre = (animated = true)=>{
    const {index} = this.state;
    if(index != 0){
      // console.info("this.state.index: ",index)
      this.scrollBy(-1,animated);
    }
  ...
  scrollNext = (animated = true)=>{
    ...
    if((index + 1) != total){
      // console.info("this.state.index: " + total + "  ",index)
      this.scrollBy(1,animated);
    }
  ...
  render () {
    ...
    return (
      <View ref={c=>this.ui=c} style={[styles.container, containerStyle]} onLayout={this.onLayout}>
